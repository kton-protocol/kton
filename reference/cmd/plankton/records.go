...
// and what `add` accepts. The fixture in reference/testdata/federation/ was generated from this
// command, but it is NOT byte-identical to what this prints: printJSON marshals a map, so Go emits
// the keys sorted (`max` before `records`) while the fixture keeps the wire order. Equality holds
// for the DOCUMENT, not for the bytes - compare it parsed. An earlier version of this comment
// claimed the two "cannot drift", which was wrong in both halves: they are not the same bytes, and
// nothing here checks them against each other.
//
// `seq` is the record's durable federation position, read from objects/.seq rather than derived
// from store order - see core.SeqMap and SPEC §12 for why that distinction is load-bearing.
